...
MemoryModuleInit ()
{
  ...
  dbprintf ('m', "Before left shift: %d, 0x%x\n",
	    MEMORY_L2_PAGE_SIZE_BITS, MEMORY_L2_PAGE_SIZE_BITS);
  dbprintf ('m', "Left shift test: %d, 0x%x\n",
...
MemoryAllocPage ()
{
  ...
  v = freepages[mapnum];
	// if v has 'n' digits (bits), then multiply bitnum by (2^n) in this for loop
...
MemoryFreePage(uint32 page)
{
  ...
  nfreepages += 1;
  dbprintf ('m',"Freed page 0x%x, %d available now.\n", page, nfreepages);
}

//----------------------------------------------------------------------
//
...
//=>#define	MEMORY_PAGE_SIZE	(1 << 16)
//=>#define	MEMORY_PAGE_SIZE	65536 // is 64KB

    int	page = addr / MEMORY_PAGE_SIZE;
    int offset = addr % MEMORY_PAGE_SIZE;

		dbprintf('m', "Got to translate logical 0x%x to page 0x%x, offset 0x%x\n",
							addr, page, offset);
