...
ProcessFork (VoidFunc func, uint32 param, char *name, int isUser)
{
  ...
  DisableIntrs ();
  dbprintf ('z', "Entering ProcessFork args=0x%x 0x%x %s %d\n", func,
	    param, name, isUser);
  ...
  if (QueueEmpty (&freepcbs)) {
		...
		if (!isUser) {
			...
			dbprintf ('p', "User process fork: No Process Slots available");
			return SPAWN_NO_SLOTS;
		}
  }
  ...
  if (newPage == 0) {
		if (!isUser) {
			...
			dbprintf ('p', "User process fork: No free pages available");
      ProcessFreeResources (pcb);
			return SPAWN_NO_RESOURCES;
		}
  }
  ...
  if (isUser) {
    ...
    while ((n = ProcessGetFromFile (fd, buf, &addr, sizeof (buf))) > 0) {
      ...
      if (MemoryCopySystemToUser (pcb, buf, addr - n, n) < n) {
      	ProcessFreeResources (pcb);
				return SPAWN_NO_RESOURCES;
			}			
    }
  ...
  dbprintf ('0', "Leaving ProcessFork with pcb (%x,%x)\n", pcb,pcbs);
  return (pcb - pcbs);
}

//----------------------------------------------------------------------
...
ProcessGetFromFile (int fd, unsigned char *buf, uint32 *addr, int max)
{
  ...
  if ((nbytes = FsRead (fd, localbuf, sizeof (localbuf) - 4)) <= 0) {
    return (0);
  }
...
ProcessWait(int pid) 
{
	PCB *pcb;
	pcb = &pcbs[pid];
	dbprintf('0',"ProcessWait: Checking %x\n",pcb->flags);
	//check if process is valid
	if ((pcb->flags & PROCESS_STATUS_MASK) == PROCESS_STATUS_FREE) {
		return -1; //invalid process (or not valid anymore)
	}
	if ((pcb->flags & PROCESS_STATUS_MASK) == PROCESS_STATUS_ZOMBIE) {
		return pid; //already completed
	}
	//process is valid
	dbprintf('0',"ProcessWait: Acquiring Lock\n");
	LockAcquire(&pcb->pLock);
	dbprintf('0',"ProcessWait: Waiting\n");
	CondWait(&pcb->pCond);
	dbprintf('0',"ProcessWait: Releasing Lock\n");
	LockRelease(&pcb->pLock);
	return pid;
}

void
ProcessSignal(PCB *pcb)
{
	dbprintf('0',"ProcessSignal: Acquiring Lock\n");
	LockAcquire(&pcb->pLock);
	dbprintf('0',"ProcessSignal: Signal\n");
	CondSignal(&pcb->pCond);
	dbprintf('0',"ProcessSignal: Releasing Lock\n");
	LockRelease(&pcb->pLock);
}

int
ProcessGetProcInfo(int prevPid, struct userPCB *upcb)
{
	struct PCB *pcb;
	int pid, done;
	...
	dbprintf('z',"PS: pid is %x\n",pid);
	while ((pid < PROCESS_MAX_PROCS ) && (!done)) {
		pcb = &pcbs[pid];
		//return all processes incl. zombie
		if ((pcb->flags & PROCESS_STATUS_FREE) != PROCESS_STATUS_FREE) {
			done = 1;
		} else {
			pid++;
		}
	...
	dstrcpy(upcb->name, pcb->name);
	upcb->pid = pid;
	upcb->flags = pcb->flags;
